...
def test_saml_with_invalid_metadata(urlopen_mock, helpers):
    """
    ...
    """
    urlopen_result_mock = helpers.get_urlopen_result_mock(200, b"invalid")
...
@patch("urllib.request.urlopen")
def test_saml_with_valid_signed_metadata(urlopen_mock, helpers):
    """
    arrange: mock the metadata contents so that they valid.
    act: access the metadata properties.
    assert: the properties are populated as defined in the metadata.
    """
    metadata = Path("tests/unit/files/metadata_signed.xml").read_bytes()
...
@patch("urllib.request.urlopen")
def test_saml_with_valid_tampered_signed_metadata(urlopen_mock, helpers):
    """
    arrange: mock the metadata contents so that they invalid.
    act: access the metadata properties.
    assert: an exception is raised.
    """
    metadata = Path("tests/unit/files/metadata_signed_tampered.xml").read_bytes()
    urlopen_result_mock = helpers.get_urlopen_result_mock(200, metadata)
    urlopen_mock.return_value = urlopen_result_mock
...
@patch("urllib.request.urlopen")
def test_saml_with_valid_signed_metadata_not_matching_fingerprint(urlopen_mock, helpers):
    """
...
@patch("urllib.request.urlopen")
def test_saml_with_valid_unsigned_metadata(urlopen_mock, helpers):
    """
    arrange: mock the metadata contents so that they invalid.
    act: access the metadata properties.
    assert: the properties are populated as defined in the metadata.
